...
public class User implements Serializable {
    ...
    // Added to support ProfileActivity. Overlaps with the flat allergies/age/
    // chronicConditions fields above (used by toProfileJsonObject() for the
    // existing emergency dispatch contract) - left as two representations
    // rather than merged; see MedicalProfile's javadoc.
     ...
     * Null fields are coalesced to empty strings so the key is always present in the payload -
     * org.json.JSONObject.put() silently omits a key when given a null value, which would
     * otherwise cause incomplete profiles (e.g. a user who triggers SOS before finishing
     * onboarding) to send a payload missing required backend fields.
     */
