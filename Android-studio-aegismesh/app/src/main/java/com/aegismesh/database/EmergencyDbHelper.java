...
public class EmergencyDbHelper extends SQLiteOpenHelper {
    ...
    /**
     * Inserts a new emergency alert or updates its fields (like status) if it
     * already exists.
     */
    ...
    /**
     * Retrieves all emergencies stored locally that are still PENDING (i.e.
     * never successfully offered to the backend or mesh). There is no separate
     * FAILED status in the current state machine -- a failed send attempt
     * simply leaves (or resets) the emergency at STATUS_PENDING, so a
     * single-condition query here is sufficient and stays in sync with
     * Emergency's 7-state lifecycle.
     */
    public synchronized List<Emergency> getUnsentEmergencies() {
        ...
        try {
            ...
            String[] selectionArgs = new String[]{Emergency.STATUS_PENDING};
